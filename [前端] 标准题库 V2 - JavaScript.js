...
function shallowCopy(src) {
  ...
  // 保留原型
  // dst.__proto__ = src.__proto__; // 非标准写法
...
function createWeightedRandom(input) {
  ...
  for (const num of input) {
    sum += num;
    arr.push(sum - 1); // sum从0开始所以这里减1
  }

  // 随机函数
  function random() {
    const randomNum = Math.floor(Math.random() * sum);
    return bisection(randomNum, 0, arr.length - 1);
  }

  // 二分法查区间
  function bisection(target, start, end) {
    if (start >= end) return start;
    const mid = Math.floor((start + end) / 2);
    if (arr[mid] < target) {
      return bisection(target, mid + 1, end);
    } else {
      return bisection(target, start, mid);
    }
  }
  return random;
}
...
function curry(fn, ...existingArgs) {
  // show me your code
  if (existingArgs.length === fn.length) return fn(...existingArgs);
  return function curried(...args) {
    if (existingArgs.length + args.length >= fn.length) return fn(...existingArgs, ...args);
